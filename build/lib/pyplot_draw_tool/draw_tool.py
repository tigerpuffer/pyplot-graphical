...
class DrawTool:
    ...
    def _init_gui(self):
        ...
        self.dataset_count = ttk.Combobox(frame_data, values=[1, 2, 3, 4, 5], width=5, state="readonly")
        ...
        self.entry_x = ttk.Entry(frame_data, width=60)
    ...
    def _create_dataset_inputs(self):
        ...
        for i in range(count):
            ...
            entry_y = ttk.Entry(frame, width=30)
            ...
            entry_err = ttk.Entry(frame, width=20)
    ...
    def _init_gui(self):
        ...
        # 颜色/样式
        tk.Label(frame_setting, text="线条颜色：", font=("微软雅黑", 11)).grid(row=2, column=0, padx=5, pady=8, sticky="w")
        self.entry_color = ttk.Entry(frame_setting, width=18)
        self.entry_color.insert(0, "blue")
        self.entry_color.grid(row=2, column=1, padx=5, pady=8)
        
        self.var_grid = tk.BooleanVar()
        chk_grid = ttk.Checkbutton(frame_setting, text="显示网格", variable=self.var_grid)
        chk_grid.grid(row=2, column=2, padx=5, pady=8)
        
        self.var_legend = tk.BooleanVar()
        chk_legend = ttk.Checkbutton(frame_setting, text="显示图例", variable=self.var_legend)
        chk_legend.grid(row=2, column=3, padx=5, pady=8)
        ...
        # 提示文字
        tk.Label(self.root, text="✅ 支持：折线/柱状/散点/误差棒/雷达/直方图/饼图 | ✅ 完全自定义样式", font=("微软雅黑", 10), fg="gray").pack(pady=5)
    ...
    def plot_data(self):
        try:
            # 1. 获取用户输入的X轴数据
            x_data = self._parse_data(self.entry_x.get(), str)

            # 2. 获取多个数据集的Y轴数据和误差数据
            datasets = []
            colors = ['blue', 'orange', 'green', 'red', 'purple']  # 默认颜色列表
            
            for i, frame in enumerate(self.dataset_frames):
                label = frame['label'].get().strip() or f"数据集 {i+1}"
                y_str = frame['y_data'].get().strip()
                err_str = frame['err_data'].get().strip()
                
                if not y_str:
                    raise ValueError(f"请输入数据集 {i+1} 的Y轴数据")
                
                y_data = self._parse_data(y_str, float)
                error = self._parse_data(err_str, float) if err_str else None
    ...
    def _parse_data(self, input_str, data_type):
        """解析输入字符串为指定类型的数据列表"""
        try:
            data = list(map(data_type, input_str.strip().split(',')))
            if not data:
                raise ValueError("数据不能为空")
            return data
        except ValueError:
            raise ValueError("请输入有效的数字，用逗号分隔")
    ...
    def fill_example(self):
        ...
        self.entry_x.insert(0, "0,2.5,5,7.5")
        
        # 填充数据集数据
        examples = [
            {"label": "0", "y_data": "0.2,1.1,0.95,0.55", "err_data": "0.05,0.1,0.08,0.06"},
            {"label": "2.5", "y_data": "0.5,1.15,0.98,0.6", "err_data": "0.06,0.12,0.09,0.07"},
            {"label": "5", "y_data": "0.48,1.0,0.96,0.58", "err_data": "0.05,0.11,0.08,0.06"},
            {"label": "7.5", "y_data": "0.45,0.95,0.94,0.56", "err_data": "0.04,0.1,0.07,0.05"}
        ]
        ...
        for i, example in enumerate(examples):
            if i < len(self.dataset_frames):
                ...
                frame['y_data'].delete(0, tk.END)
                frame['y_data'].insert(0, example["y_data"])
                frame['err_data'].delete(0, tk.END)
                frame['err_data'].insert(0, example["err_data"])
    ...
    def clear_all(self):
        """清空所有输入字段"""
        self.entry_x.delete(0, tk.END)
        for frame in self.dataset_frames:
            frame['label'].delete(0, tk.END)
            frame['label'].insert(0, f"数据集 {self.dataset_frames.index(frame)+1}")
            frame['y_data'].delete(0, tk.END)
            frame['err_data'].delete(0, tk.END)
        self.entry_title.delete(0, tk.END)
        self.entry_xlabel.delete(0, tk.END)
